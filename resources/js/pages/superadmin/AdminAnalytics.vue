...
const fetchBarangayDistribution = async () => {
  const response = await api.get('/superadmin/analytics/barangay-distribution', {
    params: getDateFilterParams(),
  })
...
const fetchAssistanceDistribution = async () => {
  ...
  try {
    ...
    const response = await api.get('/superadmin/analytics/assistance-distribution', {
      params,
    })
...
watch(selectedBarangayIdForAssistance, () => {
  if (isLoadingAnalytics.value || !selectedOffice.value) return
  fetchAssistanceDistribution()
})
