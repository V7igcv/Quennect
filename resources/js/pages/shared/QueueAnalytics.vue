<template>
  <div class="max-w-7xl mx-auto px-2 sm:px-2 lg:px-2 py-2">
    ...
    <Teleport v-if="canExportGraphs" to="body">
          ...
          <div class="relative bg-white rounded-lg shadow-2xl w-full max-w-lg p-6 z-10 mx-4">
            ...
            <div class="flex justify-end gap-3">
              ...
              <Button
                ...
                type="button"
                :disabled="isExportingGraphs || (hasOfficeFilter && !selectedOffice)"
    ...
    <Teleport v-if="canExportTable" to="body">
      <Transition name="modal-fade">
...
const canExportGraphs = computed(() => userRole.value !== 'CITY MAYOR')
const canExportTable = computed(() => userRole.value === 'SUPERADMIN')
const analyticsBasePath = computed(() => {
  if (userRole.value === 'SUPERADMIN') return '/superadmin/analytics'
  if (userRole.value === 'CITY MAYOR') return '/city-mayor/analytics'
  return '/frontdesk/analytics'
})
...
const frontdeskOfficeDetails = computed(() => {
  const storedUser = localStorage.getItem('user')
  if (!storedUser) {
    return {
      displayName: 'this office',
      acronym: '',
    }
  }
  ...
  try {
    ...
    if (office?.name && office?.acronym) {
      return {
        displayName: `${office.name} (${office.acronym})`,
    ...
    console.error('Failed to parse stored user office details', error)
  }
...
const selectedOfficeAcronym = computed(() => {
  if (!hasOfficeFilter.value) {
    return frontdeskOfficeDetails.value.acronym
  }
...
const selectedOfficeDisplayName = computed(() => {
  if (!hasOfficeFilter.value) {
    return frontdeskOfficeDetails.value.displayName
  }
...
const openExportModal = () => {
  if (!canExportGraphs.value) return
...
const confirmExportGraphs = async () => {
  if (!canExportGraphs.value) return
  if (isExportingGraphs.value || (hasOfficeFilter.value && !selectedOffice.value)) return

  isExportingGraphs.value = true

  try {
    const response = await api.get(`${analyticsBasePath.value}/export-graphs`, {
...
const confirmExportTable = async () => {
  ...
  try {
    const response = await api.get(`${analyticsBasePath.value}/queue-summary/export`, {
      params: getDateFilterParams(),
      responseType: 'blob',
    })
    ...
    console.error('Error exporting queue summary table:', error)
...
const fetchCardStats = async () => {
  const response = await api.get(`${analyticsBasePath.value}/cards`, {
    params: getDateFilterParams(),
  })
...
const fetchClientSatisfaction = async () => {
  const response = await api.get(`${analyticsBasePath.value}/client-satisfaction`, {
    params: getDateFilterParams(),
  })
...
const fetchBarangayDistribution = async () => {
  const response = await api.get(`${analyticsBasePath.value}/barangay-distribution`, {
    params: getDateFilterParams(),
  })
...
const fetchAssistanceDistribution = async () => {
  if (hasOfficeFilter.value && !selectedOffice.value) {
  ...
  try {
    ...
    const response = await api.get(`${analyticsBasePath.value}/assistance-distribution`, {
      params,
    })
...
const fetchLaneTypeDistribution = async () => {
  const response = await api.get(`${analyticsBasePath.value}/lane-type`, {
    params: getDateFilterParams(),
  })
...
const fetchQueueSummary = async () => {
  ...
  try {
    const response = await api.get(`${analyticsBasePath.value}/queue-summary`, {
      params: {
        ...getDateFilterParams(),
...
watch(currentQueueSummaryPage, () => {
  if (isApplyingDateFilter.value || (hasOfficeFilter.value && !selectedOffice.value)) return
  fetchQueueSummary()
})

watch(selectedBarangayIdForAssistance, () => {
  if (isLoadingAnalytics.value || (hasOfficeFilter.value && !selectedOffice.value)) return
  fetchAssistanceDistribution()
})
